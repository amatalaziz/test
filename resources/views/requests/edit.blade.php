                ...
                <form method="POST" action="{{ route('requests.update', $request) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" id="description" rows="4" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('description', $request->description) }}</textarea>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="priority" id="priority" required
                                ...
                                @foreach(App\Enums\RequestPriority::cases() as $priority)
                                    <option value="{{ $priority->value }}" @selected(old('priority', $request->priority) == $priority->value)>
                                        {{ $priority->label() }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="status" id="status" required
                                    ...
                                    @foreach(App\Enums\RequestStatus::cases() as $status)
                                        <option value="{{ $status->value }}" @selected(old('status', $request->status) == $status->value)>
                                            {{ $status->label() }}
                                        </option>
                                    @endforeach
                                </select>
